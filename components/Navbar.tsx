      ...
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <a href="/" className="flex items-center gap-3 group">
          ...
          <div className="flex items-baseline gap-1.5">
            <span className="font-bold text-white text-lg leading-none">FutureHive</span>
            <span className="text-slate-500 text-xs hidden sm:block font-medium uppercase tracking-wider">Portal</span>
          </div>
        </a>
        ...
        <nav className="hidden md:flex items-center gap-1">
          {navLinks.map((link) => (
            <a
              ...
              className="px-4 py-2 text-slate-400 hover:text-white text-sm font-medium transition-colors rounded-lg hover:bg-white/[0.06]"
            >
              {link.label}
            </a>
          ))}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="md:hidden overflow-hidden bg-[#050d1a]/95 backdrop-blur-md border-b border-white/[0.06]"
          >
            <div className="px-6 py-4 flex flex-col gap-1">
              {navLinks.map((link) => (
                <a
                  ...
                  className="px-4 py-2.5 text-slate-300 hover:text-white text-sm font-medium rounded-lg hover:bg-white/[0.06] transition-colors"
                >
                  {link.label}
                </a>
              ))}
